<div class="p-6 print bg-gray-100 shadow-md rounded-lg report-content bg-gray-100">
    <div class="mb-4 ">
        <ul class="list-disc list-inside mt-1 ml-5">
            @foreach ($incident->corrective_action as $action)
            <li class="mb-2">{{ $action->description }}</li>
            @endforeach
        </ul>
    </div>
    <p class="text-red-600 font-semibold">Subsanación rechazada</p>
</div>
...
<div id="modalLifting" class="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-75 z-50 hidden">
    <div class="bg-white rounded-lg shadow-lg w-11/12 md:w-1/2 lg:w-1/3">
        <div class="p-8">
        <h2 class="text-2xl font-bold mb-6 text-gray-800">Nueva fecha de subsanación</h2>
        <form method="POST" action="{{ route('incident.update', $incident) }}" class="bg-white rounded-lg ">
            ...
            <div class="flex items-center justify-end mt-6">
                <button type="button" id="closeModalBtn" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded-lg focus:outline-none focus:shadow-outline transition duration-300 ease-in-out mr-4">
                    Cancelar
                </button>
